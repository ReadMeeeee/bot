...
async def add_group(tg_id: int, name: str, number: int, link: str):
    async with async_session() as session:
        async with session.begin():
            group = Group(tg_id=tg_id, name=name, number=number, tg_link=link)
            session.add(group)
        await session.commit()

# Добавление студента
async def add_student(tg_id: int, username: str, full_name: str, is_leader: bool, course: str, group_id: int):
    async with async_session() as session:
        async with session.begin():
            student = Student(
...
async def delete_student(username: str):
    async with async_session() as session:
        async with session.begin():
            result = await session.execute(select(Student).filter_by(username=username))
...
async def update_leader(username: str):
    async with async_session() as session:
        async with session.begin():
            ...
            student = result.scalar()
            if student:
                # Снимаем старосту с остальных студентов группы
                await session.execute(
                    select(Student)
                    .filter(Student.group_id == student.group_id, Student.is_leader == True)
